...
from .base_layers import Conv_Up_block_3D_c, Conv_Up_block
...
class TBDecoder(keras.layers.Layer):
    def __init__(self, n_layers=4, filters=8, w=3, activation="swish", name="", **kwargs):
        ...
        self.tb_up = [
            Conv_Up_block_3D_c(filters / 2 * 2 ** (n_layers - i), w, act=activation, name="tb_decoder_{}".format(i))
            for i in range(n_layers - 1)
        ]

        self.tb_up_last = Conv_Up_block_3D_c(filters, w, name="decoder_{}".format(n_layers - 1))
